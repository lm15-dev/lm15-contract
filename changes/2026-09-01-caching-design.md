...
- Three tiers exist across 13 providers studied (six measured live — OpenAI both dialects, Anthropic, Gemini, xAI, Groq; seven read from docs only — wording corrected 2026-09-02 after the independent review): **automatic** (best-effort, nothing to send), **breakpoint** (a mark on a block, guaranteed above a per-model minimum, 1.25× write), **resource** (a stored, named, billed-per-hour object; Gemini only among lm15's providers). `research/caching/10-facts.md`, `30-model.md`.
...
Receipts of note: `openai.cache_off` — 3k-token prefix on gpt-5.6-sol, `cache_write_tokens == 0` (implicit mode wrote 3088 per call); `anthropic.cache_history` — turn 1 writes 3275, turn 2 reads 3275 and writes 24 ("3271" was a transcription slip; the body says 3275 — independent review 2026-09-02); `gemini.cache_resource` — `cachedContentTokenCount == 3580` against the object created in `gemini.cache`.
