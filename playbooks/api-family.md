...
**A compat preset name supplies its server's address; never the cloud's (2026-09-11, pending ratification).** `OpenAIChatLM(compat="ollama")` is `http://localhost:11434/v1`; `compat="lmstudio"` (spelling `lm-studio`) is `http://localhost:1234/v1` — LM Studio's documented default — with ollama's wire policy, its own preset key, in every dialect table that has ollama. The local engines' `/v1` roots (`ollama`, `lmstudio`, `vllm`, `sglang`) are the same on the Chat and Responses doors (one server, one root). Only the dialect's own default name (`openai`, `anthropic`) or no compat resolves to the cloud default. A preset name with no address row in the chosen dialect (`qwen` on Chat; `qwen`, `deepseek`, `zai` on Responses; the host-templated `bedrock*` outside the registry) is refused at construction with `NotConfiguredError` naming `base_url` — the request the user addressed to a named server is never sent to `api.openai.com` with whatever key is around. An explicit `base_url` always wins. Before this date the reference sent `lmstudio` to the OpenAI cloud (the name missed its address table) and the Rust and TypeScript ports sent it to ollama's port (the name was an alias of `ollama`): three implementations, two wrong answers, none the documented one.
