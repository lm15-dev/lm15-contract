User Guide — google-auth 2.57.0 documentation

User Guide¶

Credentials and account types¶

Credentials are the means of identifying an application or
...
installed, default() can automatically determine the credentials from the
environment:
...
Applications running on Compute Engine, Container Engine, or the App
Engine flexible environment can obtain credentials provided by Compute
Engine service accounts. When running on these platforms you can obtain
credentials for the service account one of two ways:

Use application default credentials.
default() will automatically detect if these credentials are available.

Use compute_engine.Credentials:
...
Applications running on the App Engine standard environment can obtain
credentials provided by the App Engine App Identity API. You can obtain
credentials one of two ways:

Use application default credentials.
default() will automatically detect if these credentials are available.

Use app_engine.Credentials:
...
If you are using the google.auth.transport.requests transport, vendor
in the requests-toolbelt library into your app, and enable the App Engine
monkeypatch. Refer App Engine documentation for more details on this.
...
can create a google.oauth2.credentials.Credentials instance:
...
google_auth_oauthlib.helpers.credentials_from_session() to obtain
google.oauth2.credentials.Credentials from a
requests_oauthlib.OAuth2Session as above:

from google_auth_oauthlib.helpers import credentials_from_session

google_auth_credentials = credentials_from_session(oauth2session)

You can also use google_auth_oauthlib.flow.Flow to perform the OAuth
...
supports OpenID Connect (OIDC), the following requirements are needed:
...
You can now use the Auth library to
call Google Cloud resources from an OIDC or SAML provider.

Accessing resources using a custom supplier with OIDC or SAML¶

This library also allows for a custom implementation of google.auth.identity_pool.SubjectTokenSupplier
to be specificed when creating a google.auth.identity_pool.Credential. The supplier must
...
should return a google.auth.exceptions.RefreshError and indicate via the error
whether the subject token retrieval is retryable.
Any call to the supplier from the Identity Pool credential will send a google.auth.external_account.SupplierContext
object, which contains the requested audience and subject type. Additionally, the credential will
send the google.auth.transport.requests.Request passed in the credential refresh call which
...
Where the audience is: ///iam.googleapis.com/projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/POOL_ID/providers/PROVIDER_ID
Where the following variables need to be substituted:
...
by generating a credential configuration file with the gcloud CLI.

Accessing resources using a custom supplier with AWS¶

This library also allows for a custom implementation of google.auth.aws.AwsSecurityCredentialsSupplier
to be specificed when creating a google.auth.aws.Credential. The supplier must
...
This is needed since default() will try to auto-discover the project ID
...
For Azure and OIDC providers, use identity_pool.Credentials.from_info or
...
For AWS providers, use aws.Credentials.from_info or
...
Workforce identity federation lets you use an external identify provider
...
Follow the detailed instructions on how to configure workforce identity
federation.
...
can be generated by using the gcloud CLI.
...
You can now use the Auth library to call Google
...
initializing the impersonated credential. Refer create short lived credentials delegated for more details on delegation chain.
...
Google Cloud Console. Once you have a private key you can obtain
credentials one of three ways:

Set the GOOGLE_APPLICATION_CREDENTIALS environment variable to the full
path to your service account private key file

$ export GOOGLE_APPLICATION_CREDENTIALS=/path/to/key.json

Then, use application default credentials.
default() checks for the GOOGLE_APPLICATION_CREDENTIALS
...
Use service_account.Credentials.from_service_account_info:
...
Private keys must be kept secret. If you expose your private key it
is recommended to revoke it immediately from the Google Cloud Console.

Downscoped credentials¶

Downscoping with Credential Access Boundaries is used to restrict the
...
Impersonated,
and Compute Engine.  These tokens can be used to
...
If your application runs on App Engine, Cloud Run, Compute Engine, or
...
google.oauth2.id_token module. It supports ID token signed with RS256
...
The recommended HTTP transport is google.auth.transport.requests which
...
urllib3 is the underlying HTTP library used by Requests and can also be
...
You can also construct your own urllib3.PoolManager instance and pass
...
gRPC is an RPC framework that uses Protocol Buffers over HTTP 2.0.
google-auth can provide Call Credentials for gRPC. The easiest way to do
...
jwt.Credentials) then you can specify None.
