...
meta:
  ...
  description: API reference for retrieving a model response with GET /v1/responses/{response_id}.
  keywords: Responses API, retrieve response, GET /responses, API reference, endpoint reference
cms:
  layout: large
  alias: /model-api/docs/api-reference/responses/retrieve-response
  target: aidmc
---
...
operation:
  ...
  tags:
  - Responses
  summary: Retrieve a stored model response by ID.
  parameters:
  - in: path
    name: response_id
    required: true
  ...
  - in: query
    ...
    schema:
      type: boolean
    description: 'If set to true, the response is streamed back as server-sent events as its events become available, terminated by a `data: [DONE]` sentinel. Streaming is only available for in-progress background responses.'
  - in: query
    name: starting_after
    required: false
    schema:
      type: integer
    description: When streaming, only events with a sequence number greater than this value are sent (used to resume a stream). Defaults to streaming from the first event.
  responses:
    '200':
      description: OK
      content:
...
For usage examples and `previous_response_id` patterns, see the [Responses](/docs/protocols/responses) feature page.