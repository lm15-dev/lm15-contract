---
layout: Conceptual
title: Use the Azure OpenAI Responses API - Microsoft Foundry | Microsoft Learn
canonicalUrl: https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/responses
...
locale: en-us
document_id: 5bb9ac79-e340-99ce-1d89-c9efdfdc0d06
document_version_independent_id: 2f97672f-4a30-6096-051c-f20c7765b84c
updated_at: 2026-08-18T22:34:00.0000000Z
...
monikers: []
item_type: Content
source_path: articles/foundry/openai/how-to/responses.md
cmProducts:
...
Use the Azure OpenAI Responses API to generate stateful, multi-turn responses. It brings together capabilities from chat completions and the Assistants API in one unified experience. Use [multi-agent orchestration](responses-multi-agent) to delegate independent work to parallel subagents, or use [tool search](tool-search) to load tool definitions only when the model needs them. The Responses API also supports the `computer-use-preview` model that powers [Computer use](../../../foundry-classic/openai/how-to/computer-use).

For complete request and response parameter descriptions, see the [Responses API parameter reference](/en-us/rest/api/microsoft-foundry/azureopenai/responses?view=rest-microsoft-foundry-v1-preview&preserve-view=true).
...
Before running the examples in this article, confirm that your resource region supports the Responses API. The v1 API is required to access the latest features. For details, see the [API version lifecycle](../api-version-lifecycle). For Foundry Agent Service regional support, see the [regional availability table](../../agents/concepts/limits-quotas-regions#supported-regions). The Responses API is currently available in the following regions:
...
Not every model is available in every supported region. Check the [models page](../../foundry-models/concepts/models-sold-directly-by-azure) for model region availability.
...
const responseEntra = await openaiEntra.responses.retrieve("");
console.log(responseEntra.output_text);
```
...
Response response = openAIClient.responses().retrieve("");
System.out.println(response.outputText());
```
...
```bash
curl -X GET https://YOUR-RESOURCE-NAME.openai.azure.com/openai/v1/responses/ \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AZURE_OPENAI_AUTH_TOKEN"
```
...
const result = await openai.responses.delete("");
console.log(result);
...
const resultEntra = await openaiEntra.responses.delete("");
...
Response result = openAIClient.responses().delete("");
...
```bash
curl -X DELETE https://YOUR-RESOURCE-NAME.openai.azure.com/openai/v1/responses/ \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AZURE_OPENAI_AUTH_TOKEN"
```
...
try (StreamResponse stream = openAIClient.responses().createStreaming(params)) {
    stream.stream()
...
During streaming, if an error occurs mid-stream, the system attempts internal retries to resolve the error. If the request still fails, the service encounters an unrecoverable error. In these cases, the HTTP response code is 200 (success), but the error event in the stream includes details about the midstream errors.

The server returns the following error types:

| Error type | Can be translated as |
| --- | --- |
| `server_error` | Return code `500` |
| `too_many_requests` | Return code `429` |
| `forbidden` | Return code `403` |
| `user_error` | Return code `400` |
...
Use structured outputs to make a response follow a JSON Schema. In a Responses API request, define the schema in `text.format`. Chat Completions uses `response_format` instead. For SDK and REST examples, supported schema constraints, and limitations, see [Structured outputs](structured-outputs).
...
ClientResult result = await openAIClient.CreateResponseAsync(options);
...
To learn more about guardrail categories and severity levels, see [Guardrails overview](../../guardrails/guardrails-overview) and [Work with annotations](../../guardrails/how-to-create-guardrails#work-with-annotations).
...
To create a container, specify `"container": { "type": "auto", "file_ids": ["file-1", "file-2"] }` in the tool configuration when creating a new Response object. This automatically creates a new container or reuses an active one from a previous code\_interpreter\_call in the model's context. The `code_interpreter_call` in the output of the API will contain the `container_id` that was generated. This container expires if it is not used for 20 minutes.
...
# [REST](#tab/rest)
```bash
curl -X GET https://YOUR-RESOURCE-NAME.openai.azure.com/openai/v1/responses//input_items \
  -H "Content-Type: application/json" \
  -H "api-key: $AZURE_OPENAI_API_KEY"
```
...
- A file ID created with the [Files API](/en-us/rest/api/microsoft-foundry/azureopenai/files?view=rest-microsoft-foundry-v1-preview&preserve-view=true)
...
CreateResponseOptions options = new()
{
    ...
    InputItems =
    {
        ...
        [
            ...
            ResponseContentPart.CreateInputImagePart(new Uri(""))
        ])
    }
};
    ...
    "input": [
      {
        "role": "user",
        "content": [
          {"type": "input_file", "filename": "PDF-FILE-NAME.pdf", "file_data": "data:application/pdf;base64,"},
          {"type": "input_text", "text": "Summarize this PDF."}
        ]
      }
    ]
...
ResponseResult priorResponse = await openAIClient.GetResponseAsync("");
...
ResponseResult current = await openAIClient.GetResponseAsync("");
...
Response current = openAIClient.responses().retrieve("");
...
ResponseResult cancelled = await openAIClient.CancelResponseAsync("");
Console.WriteLine($"Status: {cancelled.Status}");
```

# [JavaScript](#tab/javascript)
```javascript
const cancelled = await client.responses.cancel("");
...
Response cancelled = openAIClient.responses().cancel("");
System.out.println(cancelled.status());
```

# [REST](#tab/rest)
```bash
curl -X POST https://YOUR-RESOURCE-NAME.openai.azure.com/openai/v1/responses//cancel \
  -H "Content-Type: application/json" \
  -H "api-key: $AZURE_OPENAI_API_KEY"
```
...
long cursor = 0L;
try (StreamResponse stream = openAIClient.responses().retrieveStreaming(
        "",
...
If a streaming connection drops, you can resume from a known event by passing `stream=true` along with `starting_after=` on a `GET` to the response. The service replays events emitted after that sequence number.

```bash
curl -N -X GET "https://YOUR-RESOURCE-NAME.openai.azure.com/openai/v1/responses/?stream=true&starting_after=42" \
  -H "Content-Type: application/json" \
  -H "api-key: $AZURE_OPENAI_API_KEY"
```
...
The image generation tool in the Responses API is supported by `gpt-image-1`-series models, and you can call it from a set of compatible chat and reasoning models. For the current list of supported orchestration models, see the Supported models section later in this article.

The image generation tool doesn't currently support streaming mode. To stream partial images, call the [image generation API](dall-e) directly outside of the Responses API.
...
For examples of how to use reasoning models with the responses API see the [reasoning models guide](reasoning#reasoning-summary).

## Computer use

Computer use with Playwright has moved to the [dedicated computer use model guide](../../../foundry-classic/openai/how-to/computer-use#playwright-integration).
